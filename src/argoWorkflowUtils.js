...
function mapNodeToTask(node) {
    ...
    let templateName = `${baseTemplateName}-${node.info.id.replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
    
    // For plot nodes, include operation in template name
    if (node.info.type === "plot" && node.data.operation) {
        templateName = `plot-${node.data.operation}-${node.info.id.replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
    }

    // Clean task name
    let taskName = node.info.id.replace(/[^a-z0-9-]/gi, "-").toLowerCase();
    if (node.info.type === "plot" && node.data.operation) {
        taskName = `${taskName}-${node.data.operation}`.replace(/[^a-z0-9-]/gi, "-").toLowerCase();
    }
...
function createUniqueTemplate(baseTemplate, node) {
    ...
    const baseTemplateName = nodeTypeToTemplate[node.info.type] || node.info.type;
    newTemplate.name = `${baseTemplateName}-${node.info.id.replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
...
function createUniquePlotTemplate(baseTemplate, node) {
    ...
    if (node.data.operation) {
        newTemplate.name = `plot-${node.data.operation}-${node.info.id.replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
    } else {
        newTemplate.name = `plot-${node.info.id.replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
    }
...
export function generateArgoWorkflow(name, nodes) {
    ...
    nodes.forEach(node => {
        ...
        if (node.info.type === "plot") {
            ...
            if (baseTemplate) {
                const uniqueTemplate = createUniquePlotTemplate(baseTemplate, node);
