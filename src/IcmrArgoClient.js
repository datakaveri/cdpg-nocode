	...
	async submitWorkflow(workflow) {
		...
		try {
			...
			const response = await fetch(
				`${this.baseUrl}/api/v1/workflows/argo`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${this.token}`,
			...
			if (!response.ok) {
				console.error(
					`Workflow submission error (${response.status}):`,
					responseData
				);
				throw new Error(`HTTP ${response.status}: ${responseData}`);
			}
			...
			try {
				...
				if (jsonResponse.metadata && jsonResponse.metadata.name) {
					console.log(
						`Actual workflow name: ${jsonResponse.metadata.name}`
					);
					return jsonResponse;
				} else {
					console.warn(
						"Workflow submission response does not contain the workflow name:",
						jsonResponse
					);
					return jsonResponse;
				}
